...
int CImageEvaluationDlg::EncoderVideo(Mat& img_src, uint8_t* m_pBitstream, long long	pts)
{
	...
	int encoderCodec = ESMNvenc::VIDEO_CODEC_T::HEVC;
	int encoderProfile = ESMNvenc::HEVC_PROFILE_T::MP;
#elif defined(WITH_AVC_TEST)
	int encoderCodec = ESMNvenc::VIDEO_CODEC_T::AVC;
	int encoderProfile = ESMNvenc::AVC_PROFILE_T::BP;
#else
	int encoderCodec = ESMNvenc::VIDEO_CODEC_T::AVC;
	int encoderProfile = ESMNvenc::AVC_PROFILE_T::BP;
#endif
	int encoderBitrate = ((outputWidth * outputHeight) <= (1920 * 1080)) ? 1024 * 1024 * bitratesize : 1024 * 1024 * 75;
	// 5,10,15,20
	...
	if (m_nvEncoder == NULL) {
		...
		//cudaMallocPitch(&cuDeviceptr1, &pitch1, outputWidth, outputHeight * 1.5);
	}
