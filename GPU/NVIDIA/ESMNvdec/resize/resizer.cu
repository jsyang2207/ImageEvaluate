...
static void resize(unsigned char * dst, unsigned char * dstChroma, int dstPitch, int dstWidth, int dstHeight, unsigned char * src, int srcPitch, int srcWidth, int srcHeight)
{
	...
	resize<uchar2><<<dim3((dstWidth + 31) / 32, (dstHeight + 31) / 32), dim3(16, 16)>>>(texY, texUV, dst, dstChroma, dstPitch, dstWidth, dstHeight, 1.0f * dstWidth / srcWidth, 1.0f * dstHeight / srcHeight);

	cudaDestroyTextureObject(texY);
	cudaDestroyTextureObject(texUV);
}

void resizer::resize_nv12(unsigned char * dstNV12, int dstNV12Pitch, int dstNV12Width, int dstNV12Height, unsigned char * srcNV12, int srcNV12Pitch, int srcNV12Width, int srcNV12Height)
{
