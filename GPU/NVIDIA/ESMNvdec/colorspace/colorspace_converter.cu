...
void converter::convert_nv12_to_bgra32(unsigned char * nv12, int nv12Pitch, unsigned char * bgra, int bgraPicth, int width, int height)
{
	if((width * height) >= (RESOLUTION_W4K * RESOLUTION_H4K))
		setup_mat_yuv2rgb(converter::standard_t::bt2020);
	else if((width * height) >= (RESOLUTION_WFHD * RESOLUTION_HFHD))
		setup_mat_yuv2rgb(converter::standard_t::bt709);
	else
		setup_mat_yuv2rgb(converter::standard_t::bt601);
	yuv_to_rgb_kernel<<<dim3((width + 63) / 32 / 2, (height + 3) / 2 / 2), dim3(32, 2)>>>(nv12, nv12Pitch, bgra, bgraPicth, width, height);
}

void converter::convert_nv12_to_i420(unsigned char * nv12, int nv12Pitch, unsigned char * i420, int i420Pitch, int width, int height)
{
...
void converter::convert_nv12_to_yv12(unsigned char * nv12, int nv12Pitch, unsigned char * yv12, int yv12Pitch, int width, int height)
{
...
void converter::setup_mat_yuv2rgb(int imat)
{
	float wr;
	float wb;
	int black;
...
void converter::constants(int mat, float & wr, float & wb, int & black, int & white, int & max)
{ 
	// Default is BT709
	wr = 0.2126f; 
	wb = 0.0722f;
	...
	white = 235;
	max = 255;
	if (mat == converter::standard_t::bt601) 
	{
		wr = 0.2990f; 
		wb = 0.1140f;
	}
	else if (mat == converter::standard_t::bt2020)
	{
		wr = 0.2627f; 
		wb = 0.0593f;
		// 10-bit only
